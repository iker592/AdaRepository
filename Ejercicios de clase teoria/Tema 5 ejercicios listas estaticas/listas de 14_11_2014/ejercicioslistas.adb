...
PROCEDURE EjerciciosListas IS
   ...
   FUNCTION Esta (L: IN T_Lista; Num: IN Integer) RETURN Boolean IS
    -- post: devuelve true sii Num está en la lista L
      I : Natural := 1;
   BEGIN
      WHILE I <= L.Cont AND THEN L.Numeros(I) /= Num LOOP   --AND THEN porque si I vale lo mismo que L.cont y todavia no ha encontrado Num
         I:= I+1;                                           --En la siguiente vuelta vale 11 y si el programa "consulta" L.Numeros (11) da CONSTRAINT ERROR
      END LOOP;
      RETURN ( I <= L.Cont ); -- Devuelve TRUE sii I vale 10 o menos ya que si no encuentra Num I valdrá 11 porque recorre la lista entera.
   END Esta;
   -------------------------------------------------------------
   ...
   FUNCTION Esta_En_Lista_Ord (L: T_Lista; Num: Integer) RETURN Boolean IS
    ...
    -- post: devuelve true sii Num está en la lista L
      I : Natural := 1;
   BEGIN
      WHILE I <= L.Cont AND THEN L.Numeros(I)<Num LOOP   --mientras sea menor o igual que L.cont y el numero a consultar siga siendo menor que Num LOOP
         I:= I+1;
      END LOOP;
      RETURN ( I<=L.Cont AND THEN L.Numeros(I)=Num );    --Devuelve False si I es mayor que L.cont y no consulta lo otro porque se sale de rango
   END Esta_En_Lista_Ord;                                --Devuelve TRUE si I es menor o igual que L.cont y el numero a consultar vale = Num
   -------------------------------------------------------------
